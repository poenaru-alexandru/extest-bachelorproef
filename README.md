...
```bash
pip install -r extraction_framework/requirements.txt
```
...
Plaats GGUF-modelbestanden in de map `local_models/` in de root van de repo:
...
Kopieer `extraction_framework/.env.example` naar `extraction_framework/.env` en vul in:
